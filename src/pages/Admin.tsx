  ...
  const loadVisitorData = () => {
    ...
    setTimeout(() => {
      ...
      toast({
        ...
        description: `Loaded data for ${allVisitors.length} visitors`
      });
    }, 500); // Simulated loading for effect
  };
  ...
  const StatCard = ({ title, value, icon, color }: 
    ...
    <Card className={`dashboard-card border-${color}/30 bg-cyber-dark hover:bg-cyber-light/10 transition-all duration-300`}>
      ...
      <CardContent>
        ...
        <p className="text-xs text-cyber-text mt-1 opacity-70">Updated just now</p>
      </CardContent>
    </Card>
  );
        ...
        <div className="max-w-6xl mx-auto">
          <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
            ...
            <Button 
              ...
              disabled={loading}
            >
              {loading ? <Activity className="animate-spin mr-2 h-4 w-4" /> : <Activity className="mr-2 h-4 w-4" />}
              Refresh Data
            </Button>
          </div>
